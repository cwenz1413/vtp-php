...
class db3
{
	 ...
	 * @param whr mix 待删除的条件：数组(优先)或字串
	...
	*/
	public function P($sql, $row=array(), &$pgInf=[])
	{
		//显示的条数 
		if(isset($_GET['sn']) && $_GET['sn']>0)
		{
			$pgInf['show']=$_GET['sn']-0;
		}
		else
		{
			if(isset($pgInf['show'])==false)
				$pgInf['show']=5;
			else
				$pgInf['show']-=0;		
		}
		...
		$pgInf['total']=$this->R($sql_t, $row, 0)-0;//总条数
		//总页数= 向上取整(总条数/显示条数) 
	 ...
	 * 20180614
	 */
	public function get_one($sql, $row=array())
	{
		return $this->R($sql, $row, 1);
	}
	 ...
	 */
	public function R($sql, $row=array(), $fetchType=2, $rType='object')
	{
		//执行sql语句
		$sth=$this->get_sth($sql, $row);
		if(!$sth) return false;
		...
		switch($fetchType)
		{
			//返回 二维数组
			case 2:
				return $sth->fetchAll( $this->get_rtnType($rType) );
			//返回 一维数组
			case 1:
				return $sth->fetch( $this->get_rtnType($rType) );
			//返回 单值
			default:
	 ...
	 * lm: 加入错误判断 20180614
	 */
	public function exc($sql, $row=array())
	{
		//执行sql语句
		$sth=$this->get_sth($sql, $row);
		//返回影响条数
	...
	//执行sql语句，返回pdo::sth对象
	private function get_sth($sql, $row)
	{
